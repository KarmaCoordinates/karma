...
amount_of_knowledge_df = pd.DataFrame(amount_of_knowledge, columns =['knowledge', 'knowledge_weight']) 
amount_of_knowledge_df['knowledge_scaled_weight'] = scale(amount_of_knowledge_df['knowledge_weight'])


disciplines = [['medical', 1.5], ['engineering/technology', 1.3], ['arts/entertainment', 1.2], ['business/finance/accounting', 1.1], ['legal', 1]]
...
spiritualities_df = pd.DataFrame(spiritualities, columns =['spirituality', 'spirituality_weight']) 
spiritualities_df['spirituality_scaled_weight'] = scale(spiritualities_df['spirituality_weight'])

positive_emotions = [['daily', 2], ['often', 1.5], ['rarely', 1.2], ['never', 1]]
positive_emotions_df = pd.DataFrame(positive_emotions, columns =['positive_emotion', 'positive_emotion_weight']) 
positive_emotions_df['positive_emotion_scaled_weight'] = scale(positive_emotions_df['positive_emotion_weight'])

negative_emotions = [['daily', 2], ['often', 1.5], ['rarely', 1.2], ['never', 1]]
negative_emotions_df = pd.DataFrame(negative_emotions, columns =['negative_emotion', 'negative_emotion_weight']) 
negative_emotions_df['negative_emotion_scaled_weight'] = scale(negative_emotions_df['negative_emotion_weight'])


def calculate_level(row):
    return row.knowledge_scaled_weight + row.discipline_scaled_weight + row.diet_scaled_weight + row.workout_scaled_weight - row.drink_scaled_weight + row.spirituality_scaled_weight - row.positive_emotion_scaled_weight - row.negative_emotion_scaled_weight

# Perform the Cartesian product using merge
df = pd.merge(negative_emotions_df, 
        pd.merge(positive_emotions_df, 
            pd.merge(spiritualities_df, 
                pd.merge(drinks_df, pd.merge(workouts_df, 
                    pd.merge(diets_df, 
                        pd.merge(amount_of_knowledge_df, disciplines_df, how="cross"), 
                        how="cross"), 
                    how="cross"),
                how="cross"),
            how="cross"),
