...
def take_quiz():    
    ...
    if not st.session_state['quiz']:
        ...
        with plh.container():        
            ...
            answer1 = st.selectbox(label=f'''I can't see the emoji clearly: {grin_emoji}.''', placeholder=mem, options=['Ignorance', 'Incapacity', 'Complecency', 'Siddhi'], key="q_cannot_see_clearly", index=None)
            answer2 = st.selectbox(label=f'''I see an unhappy face: {grin_emoji}.''', placeholder=mem, options=['Ignorance', 'Incapacity', 'Complecency', 'Siddhi'], key="q_unhappy_face", index=None)
            answer3 = st.selectbox(label=f'''I see a happy face: {grin_emoji}.''', placeholder=mem, options=['Ignorance', 'Incapacity', 'Complecency', 'Siddhi'], key="q_happy_face", index=None)
            answer4 = st.selectbox(label=f'''I am not interested in knowing.''', placeholder=mem, options=['Ignorance', 'Incapacity', 'Complecency', 'Siddhi'], key="q_donot_want_to_answer", index=None)
        ...
        if st.session_state.q_cannot_see_clearly == 'Incapacity' and  st.session_state.q_unhappy_face == 'Ignorance' and st.session_state.q_happy_face == 'Siddhi' and st.session_state.q_donot_want_to_answer == 'Complecency':                    
